...
test.describe.serial("POST /user/createWithArray API Tests", () => {
    ...
    test("Should create multiple users successfully", async ({ apiFixture }) => {
        await test.step("Send POST request to create multiple users", async () => {
            ...
            const response = await apiFixture.post(`user/createWithArray`, {
                data: usersPayload,
            });
    ...
    test("Should return error when sending an empty user array", async ({ apiFixture }) => {
        await test.step("Send POST request with an empty array", async () => {
            const emptyArrayPayload = [""];

            const response = await apiFixture.post(`user/createWithArray`, {
                data: emptyArrayPayload,
            });
